...
type expr =
  ...
  | ETuple of expr list
  | ETuple_Proj of expr * int
  | ERecord of expr StrMap.t
  | ERecord_Proj of expr * string
...
let rec show_ty = function
  ...
  | TyTuple tys ->
      Printf.sprintf "(%s)" (String.concat ", " (List.map show_ty tys))
  | TyRecord fields ->
      let bindings = StrMap.bindings fields in
      Printf.sprintf "{%s}"
        (String.concat ", " (List.map show_binding bindings))
  | TyVariant ctors ->
...
let e_children = function
  ...
  | ERecord_Proj (record, _) -> [ record ]
  | EVariant (name, e, ty) -> [ e ]
  | ECase (e, handlers) ->
...
let validate_no_shadow e =
  let rec aux vars e =
    ...
    | ETuple _ | ETuple_Proj _ | ERecord _ | ERecord_Proj _ | EVariant _
    | ECase _ ->
        List.iter (aux vars) (e_children e)
    | EAbs (x, _ty, _e) when StrSet.mem x vars -> raise (Shadowed x)
    | EAbs (x, _ty, e) ->
...
let type_of e =
  let rec aux ctx = function
    ...
    | EIf (cond, branch1, branch2) -> (
        match aux ctx cond with
        | TyBool ->
            let branch1_ty = aux ctx branch1 in
            let branch2_ty = aux ctx branch2 in
            if branch1_ty == branch2_ty then branch1_ty
            else raise (Type_Mismatch (branch1_ty, branch2_ty))
        | other -> raise (Type_Mismatch (TyBool, other)))
    | EVar s -> (
        match StrMap.find_opt s ctx with
        | Some ty -> ty
        | None -> failwith ("unbound " ^ s))
    | EAbs (x, ty, expr) ->
        let ctx = StrMap.add x ty ctx in
        aux ctx expr
    | EApp (e1, e2) -> (
        let t2 = aux ctx e2 in
        match aux ctx e1 with
        | TyArr (domain, codomain) when ty_eq domain t2 -> codomain
        | TyArr (domain, _codomain) -> raise (Type_Mismatch (domain, t2))
        | other -> raise (Expected_Fun_Type other))
    | ESeq (e1, e2) ->
        let _ = aux ctx e1 in
        aux ctx e2
    | EAs (e, expected_ty) ->
    ...
    | ERecord fields ->
        ...
        TyRecord bindings
    | ETuple_Proj (e, index) -> (
        match aux ctx e with
        | TyTuple tys -> List.nth tys index
        | ty -> raise (Expected_Tuple_Type ty))
    | ERecord_Proj (e, field) -> (
        match aux ctx e with
        | TyRecord fields -> StrMap.find field fields
        | ty -> raise (Expected_Record_Type ty))
    | EVariant (ctor_name, e, variant_ty) -> (
        let ctor_ty = aux ctx e in
        match variant_ty with
        | TyVariant bindings -> (
            match StrMap.find_opt ctor_name bindings with
            | Some expected_ctor_ty when ty_eq expected_ctor_ty ctor_ty ->
                variant_ty
            | _ ->
                raise
                  (Expected_Variant_Type_W_Ctor
                     ((ctor_name, ctor_ty), variant_ty)))
        | _ ->
            raise
              (Expected_Variant_Type_W_Ctor ((ctor_name, ctor_ty), variant_ty)))
    | ECase (variant, handlers) -> (
        let fst (x, _) = x in
        let keys m = StrSet.of_seq @@ Seq.map fst @@ StrMap.to_seq m in
        let handler_ctors = keys handlers in
        match aux ctx variant with
        | TyVariant bindings as variant_ty -> (
            ...
            if handler_ctors <> variant_ctors then
              ...
              match StrMap.bindings handlers with
              | [] ->
              ...
              | (ctor_name, handler) :: tl ->
                  ...
                  expected_ty)
        | other -> raise (Expected_Variant_Type_W_Ctors (handler_ctors, other)))
  in
...
let assert_ty e expected_ty =
  ...
  if actual_ty <> expected_ty then
    failwith
      (Printf.sprintf "got %s, expected %s" (show_ty actual_ty)
         (show_ty expected_ty))
...
let check_fails_shadow e x =
  ...
  | ty ->
      failwith
        (Printf.sprintf "expected exception for shadowing, got %s" (show_ty ty))
...
let check_fails_ty_mismatch e =
  ...
  | ty ->
      failwith
        (Printf.sprintf "expected exception for type mismatch, got %s"
           (show_ty ty))
...
let _ =
  ...
  assert_ty (ETuple_Proj (tuple, 0)) TyBool;
  assert_ty (ETuple_Proj (tuple, 1)) TyUnit;
  let record =
    ERecord (map_of_pairs [ ("a", ETrue); ("b", EUnit); ("c", EFalse) ])
  in
  assert_ty record
    (TyRecord (map_of_pairs [ ("a", TyBool); ("b", TyUnit); ("c", TyBool) ]));
  assert_ty (ERecord_Proj (record, "a")) TyBool;
  assert_ty (ERecord_Proj (record, "b")) TyUnit;
  let variant =
    EVariant
      ("a", ETrue, TyVariant (map_of_pairs [ ("a", TyBool); ("b", TyUnit) ]))
  in
  let handlers =
    map_of_pairs [ ("a", EIf (EVar "a", EUnit, EUnit)); ("b", EVar "b") ]
  in
  ...
  (match type_of case_with_extra with
